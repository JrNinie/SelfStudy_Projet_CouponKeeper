package com.jr.couponkeeperapispringboot.common.security;
...
public class AuthCheckInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if(StringUtils.isEmpty(token)){
            throw new Exception("We don't find " + Constants.TOKEN_STRING + " related info in Header");
        }
        //如果key名字正确，但是value不正确
        if(!token.equals(Constants.TOKEN)){
            //这里千万别写成Constants.TOKEN了，否则就泄露了token的信息了
            throw new Exception("In Header, the value of " + Constants.TOKEN_STRING + "is wrong");
        }
